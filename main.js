...
function searchUser(e) {
  ...
  fetch(`https://api.github.com/users/${user}`)
    ...
    .then((user) => {
      ...
      if (`${user.login}` === "null" || `${user.login}` === "undefined") {
        ...
        uDoJ.innerHTML = `<span><i class="fa fa-calendar"></i></span>&ensp; Joined ${new Date(
          user.created_at
        ).toDateString()}`;
        if (`${user.bio}` === "null") {
  ...
  fetch(
    `https://api.github.com/users/${user}/repos?per_page=5&sort=created:desc`
  )
    .then((Response) => {
      console.log(Response.status);
      if (!Response.ok) {
        ...
        output += `<div class="row"> 
                ...
                <div class="card-content" style="padding:5px;">
                    ...
                    <div class="row" style="padding: 10px;">
                        <div><i class="fa fa-circle" style="color:orangered;"></i> ${
                          data.language
                        }</div>
                        <div><i class="fa fa-star-o"></i> ${
                          data.stargazers_count
                        }</div>
                        <div><i class="fa fa-code-fork"></i> ${
                          data.forks_count
                        }</div>
          ...
          datasets: [
            {
              ...
              backgroundColor: ["#dc143c", "gold", "#d3d3d3"],
              borderColor: ["#dc143c", "gold", "#d3d3d3"],
              borderWidth: 1,
              hoverBorderWidth: 3,
              hoverBorderColor: ["#dc143c", "gold", "#d3d3d3"],
            },
          ],
        },
        options: {
